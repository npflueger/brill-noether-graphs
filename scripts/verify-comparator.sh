...
# Immutable verifier pins. The lean4export revision targets Lean v4.33.0.
...
for config in "${comparator_configs[@]}"; do
  python3 - "$repository_root/$config" <<'PY'
import json
import pathlib
import sys
...
if not isinstance(config, dict) or config.get("enable_nanoda") is not True:
    print(f"error: {config_path}: enable_nanoda must be exactly true", file=sys.stderr)
    raise SystemExit(1)
PY
done
